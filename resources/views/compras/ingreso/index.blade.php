				...
				<table class="table table-striped table-bordered table-condensed table-hover">
					...
					@foreach ($ingreso as $ing)
						<tr>
							...
							<td>{{$ing->nombre}}</td>
							<td>{{$ing->tipo_comprobante. ': '.$ing->serie_comprobante.'-'.$ing->num_comprobante}}</td> <!--Concatenación de los elementos descriptivos del comprobante a fin de evitar texto en varias líneas-->
							<td>{{$ing->impuesto}}</td>
							<td>{{$ing->total}}</td>
							<td>{{$ing->estado}}</td>
							<td>
